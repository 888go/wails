...
const (
	DWM_BB_ENABLE                = 0x00000001 //     已为 fEnable 成员指定了一个值。
	DWM_BB_BLURREGION            = 0x00000002 //     已为 hRgnBlur 成员指定了一个值。
	DWM_BB_TRANSITIONONMAXIMIZED = 0x00000004 //     已为 fTransitionOnMaximized 成员指定了一个值。
)

// 以下标志用于DwmEnableComposition函数，
// 以更改桌面窗口管理器(DWM)组合状态。
const (
	DWM_EC_DISABLECOMPOSITION = 0 //     禁用组合
...
// 用于指示其成员中哪些包含有效信息。
const (
	DWM_TNP_RECTDESTINATION      = 0x00000001 //    已为rcDestination成员指定了一个值
	DWM_TNP_RECTSOURCE           = 0x00000002 //    已为rcSource成员指定了一个值
	DWM_TNP_OPACITY              = 0x00000004 // 已为 opacity 成员指定了一个值
	DWM_TNP_VISIBLE              = 0x00000008 // 已为 fVisible 成员指定了一个值
	DWM_TNP_SOURCECLIENTAREAONLY = 0x00000010 //    已为 fSourceClientAreaOnly 成员指定了一个值
)
